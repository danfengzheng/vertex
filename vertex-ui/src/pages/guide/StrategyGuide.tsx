...
const INDICATOR_DETAILS: IndicatorDetail[] = [
  ...
  },
  {
    ...
    category: '趋势', categoryColor: 'blue', signalDir: '双向',
    buyCondition:  'MACD 柱状图从负值翻正（快线上穿慢线，金叉）',
    sellCondition: 'MACD 柱状图从正值翻负（快线下穿慢线，死叉）',
    neutralCondition: '无穿越（柱状图方向未改变）',
    notes: ['只在发生穿越时产生信号，大多数 K 线返回 NEUTRAL；适合捕捉趋势转折点'],
    filterMode: 'direction-or-value', filterModeDesc: '推荐方向校验，也可使用 histogram 值',
    filterExamples: [
      { desc: 'MACD 多头确认（金叉）', condition: '方向校验（无条件）', applyTo: '双向' },
      { desc: '买入时柱状图须为正',     condition: 'histogram > 0',     applyTo: '仅买入' },
      { desc: '卖出时柱状图须为负',     condition: 'histogram < 0',     applyTo: '仅卖出' },
    ],
    votingTip: 'MACD 是趋势确认主力，建议作为高权重（40-60）投票指标；结合 MA/EMA 效果最佳',
    hardFilterTip: '方向校验模式效果良好：仅在 MACD 发生金叉/死叉时才允许开仓，过滤震荡期无效信号',
  },
  {
  ...
  },
  {
    ...
    category: '震荡', categoryColor: 'orange', signalDir: '双向',
    buyCondition:  'K 线从下方上穿 D 线（金叉）',
    sellCondition: 'K 线从上方下穿 D 线（死叉）且 K ≤ 80（超买区死叉被抑制）',
    neutralCondition: '无穿越，或 K > 80 时发生死叉（超买区抑制卖出信号）',
    notes: [
      '超买区（K > 80）的死叉会被抑制为 NEUTRAL，避免在强趋势中过早平仓',
      'K、D、J 值可通过数值条件检查当前的超买超卖程度',
    ],
    filterMode: 'direction-or-value', filterModeDesc: '方向校验或数值条件均可',
    filterExamples: [
      { desc: '买入时 K 值处于超卖区',  condition: 'k < 20',             applyTo: '仅买入' },
      { desc: '卖出时 K 值处于超买区',  condition: 'k > 80',             applyTo: '仅卖出' },
      { desc: '方向校验（发生穿越）',   condition: '方向校验（无条件）', applyTo: '双向' },
    ],
    votingTip: '短周期（1m-15m）震荡行情中效果好；与 EMA/VWAP 搭配过滤方向',
    hardFilterTip: '数值条件：买入时 k < 20（超卖），卖出时 k > 80（超买），分别设置 applyTo 精确控制',
  },
  {
  ...
  },
  {
    ...
    category: '趋势', categoryColor: 'blue', signalDir: '双向',
    buyCondition:  '收盘价 > VWAP 且正偏离 ∈ (0, deviationPct%]（突破但未过度偏离）',
    sellCondition: '收盘价 < VWAP 且负偏离 ∈ [-deviationPct%, 0)（跌破但未过度偏离）',
    neutralCondition: '价格在 VWAP 上下 0% 处（贴线），或偏离超过阈值（强弩之末，避免追入）',
    notes: [
      'deviationPct 控制有效突破范围，默认 0.2%；超出则认为追高/追空风险大，返回 NEUTRAL',
      '适合日内交易；跨日 VWAP 含义不同，建议配合日线重置逻辑使用',
    ],
    filterMode: 'direction-or-value', filterModeDesc: '推荐方向校验',
    filterExamples: [
      { desc: '价格须在 VWAP 之上做多', condition: '方向校验（无条件）', applyTo: '双向' },
      { desc: '偏离不超过 1%（不追高）', condition: 'deviation < 1.0',  applyTo: '双向' },
    ],
    votingTip: '日内突破策略中与 VOL_CONFIRM、MACD 搭配，确认放量且不过度偏离',
    hardFilterTip: '方向校验效果良好：仅在价格有效突破 VWAP 时才产生信号，自动过滤强弩之末',
  },
  {
    code: 'STOCH_RSI', nameZh: '随机RSI',
    category: '震荡', categoryColor: 'orange', signalDir: '双向',
    buyCondition:  'K 线上穿 D 线（金叉）且 K < 20（处于超卖区）',
    sellCondition: 'K 线下穿 D 线（死叉）且 K > 80（处于超买区）',
    neutralCondition: '无穿越，或穿越发生在中性区间（20-80），不在极值区',
    notes: [
      'StochRSI 只在极值区（K < 20 或 K > 80）触发信号，中间区不产生信号',
      '比 RSI 更灵敏，在振荡行情中假信号较多，建议结合趋势指标过滤',
    ],
    filterMode: 'direction-or-value', filterModeDesc: '方向校验或数值条件均可',
    filterExamples: [
      { desc: '买入时 K 须深度超卖',   condition: 'stochRsiK < 15',    applyTo: '仅买入' },
      { desc: '卖出时 K 须深度超买',   condition: 'stochRsiK > 85',    applyTo: '仅卖出' },
    ],
    votingTip: '短周期高频信号指标，与 WR、BOLL 搭配进行短线反转交易',
    hardFilterTip: '方向校验已包含极值区过滤；若需更严格门槛，可用数值条件（stochRsiK < 15 / > 85）',
  },
  {
  ...
  },
  {
    ...
    category: '趋势', categoryColor: 'blue', signalDir: '双向',
    buyCondition:  'ADX > trendThreshold（默认 25）且 +DI > -DI（多头趋势足够强）',
    sellCondition: 'ADX > trendThreshold 且 -DI > +DI（空头趋势足够强）',
    neutralCondition: 'ADX ≤ trendThreshold（市场无明确趋势）',
    notes: [
      'ADX 只衡量趋势强度，不衡量方向；+DI/-DI 决定方向',
      'ADX 是最佳的双向通用硬性过滤器：确保在有趋势时才开仓',
    ],
    filterMode: 'direction-or-value', filterModeDesc: '推荐数值条件（双向通用）',
    filterExamples: [
      { desc: '趋势足够强才开仓（推荐）', condition: 'adx >= 25',          applyTo: '双向' },
    ...
    hardFilterTip: '✅ 最适合作为双向通用前置门槛：adx >= 25 确保市场有趋势方能开仓，震荡期（ADX < 25）直接跳过复合计算',
  },
  {
    code: 'SUPERTREND', nameZh: '超级趋势',
    category: '趋势', categoryColor: 'blue', signalDir: '双向',
    buyCondition:  '趋势从下降翻转为上升（SAR 点从价格上方切换到下方）',
    sellCondition: '趋势从上升翻转为下降（SAR 点从价格下方切换到上方）',
    neutralCondition: '无趋势翻转（持续处于同一方向）',
    notes: [
      '输出 4 个字段：',
      'trend：方向标志，1.0 = 上升趋势，-1.0 = 下降趋势（最适合数值条件）',
      'superTrend：趋势线当前价位（上升时 = lowerBand，下降时 = upperBand）',
      'upperBand：上轨（当前 ATR × multiplier 动态阻力位）',
      'lowerBand：下轨（当前 ATR × multiplier 动态支撑位）',
      '⚠️ superTrend / upperBand / lowerBand 是绝对价格值，随市场波动，不适合填写固定阈值条件；推荐使用 trend 字段',
      'SuperTrend 是趋势跟踪硬过滤的首选指标之一',
      '持仓期间 trend 持续保持方向，不会在趋势内部产生反向信号',
    ],
    filterMode: 'direction-or-value', filterModeDesc: '方向校验或 trend 字段，均效果出色',
    filterExamples: [
      { desc: '确认上升趋势才做多（推荐）', condition: 'trend > 0',          applyTo: '仅买入' },
      { desc: '确认下降趋势才做空（推荐）', condition: 'trend < 0',          applyTo: '仅卖出' },
      { desc: '方向校验（仅在翻转时触发）', condition: '方向校验（无条件）', applyTo: '双向' },
    ],
    votingTip: '高权重趋势跟踪指标（60%），配合 VOL_CONFIRM 或 ADX 过滤假突破',
    hardFilterTip: '✅ 最推荐的方向硬过滤器：使用 trend 数值条件分别限制买卖方向（trend > 0 仅买入，trend < 0 仅卖出）。方向校验模式同样出色（只在趋势翻转时产生信号）。⚠️ 不要使用 superTrend/upperBand/lowerBand 作为固定阈值条件，它们是绝对价格，随市场变化',
  },
  {
    code: 'VOL_CONFIRM', nameZh: '成交量确认',
    category: '成交量', categoryColor: 'green', signalDir: '双向',
    buyCondition:  'volRatio > volMultiplier（默认 1.5）且收盘价上涨',
    sellCondition: 'volRatio > volMultiplier 且收盘价下跌',
    neutralCondition: 'volRatio ≤ volMultiplier（量不够大，信号不可靠）',
    notes: [
      'volRatio = 当前成交量 / N 日平均成交量',
      '适合过滤假突破：有方向信号但成交量不够时过滤掉',
    ],
    filterMode: 'direction-or-value', filterModeDesc: '推荐 volRatio 数值条件（双向）',
    filterExamples: [
      { desc: '放量才允许开仓（推荐）', condition: 'volRatio > 1.5',      applyTo: '双向' },
      { desc: '突破时要求更大放量',     condition: 'volRatio > 2.0',      applyTo: '双向' },
    ],
    votingTip: '配合趋势指标（MACD、SuperTrend）使用，成交量放大时为信号加分',
    hardFilterTip: '✅ 推荐作为双向通用前置门槛：volRatio > 1.5 确保放量才进入分析；不放量则跳过复合投票，有效过滤假突破',
  },
  {
