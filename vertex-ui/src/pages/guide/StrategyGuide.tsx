...
const INDICATOR_DETAILS: IndicatorDetail[] = [
  ...
  },
  {
    ...
    notes: [
      '输出字段：bearishDivergence（1.0=顶背离出现，0.0=无）、bullishDivergence（1.0=底背离出现，0.0=无）',
      '顶背离（bearishDivergence=1）：价格高点升高，RSI 高点降低 → 上涨动能衰竭，卖出信号',
      '底背离（bullishDivergence=1）：价格低点降低，RSI 低点升高 → 下跌动能衰竭，买入信号',
      'swingStrength 越大（3-5）信号越稳定、越少但越可靠；越小（1-2）信号越灵敏但噪音多',
      '⚠️【最佳用法】作为出场指标：顶背离出现时平多仓（exitIndicatorConfigs 中配置 sellConditions: bearishDivergence ≥ 1）',
      '⚠️【不建议】单独作为入场主力指标；背离信号出现后价格可能仍惯性延续，建议与趋势指标配合确认',
    ],
    filterMode: 'value-only', filterModeDesc: '必须使用数值条件',
    filterExamples: [
      { desc: '出现顶背离时平多（出场指标 sellConditions）', condition: 'bearishDivergence ≥ 1', applyTo: '仅卖出' },
      { desc: '出现底背离时入场做多（买入条件）',             condition: 'bullishDivergence ≥ 1', applyTo: '仅买入' },
    ],
    votingTip: '建议专用于出场指标（exitIndicatorConfigs）：顶背离出现即触发平多。作为入场指标时权重不宜过高（≤30），需配合趋势指标确认',
    hardFilterTip: '使用数值条件模式：bearishDivergence ≥ 1（仅卖出），bearishDivergence 输出值为 1.0（有信号）或 0.0（无信号），可用 GTE + 1.0 精确匹配',
  },
];
